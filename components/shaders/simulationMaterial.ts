    ...
    super({
      ...
      fragmentShader: /* glsl */ `
        ...
        void main() {
          ...
          float waveY = sin(originalPos.x * 2.5 + continuousTime) * cos(originalPos.z * 2.5 + continuousTime * 0.7) * 0.15;
          vec3 distortion = vec3(displacementX, displacementY + waveY, displacementZ) * uNoiseIntensity;
          gl_FragColor = vec4(originalPos + distortion, 1.0);
        }
      `,
      uniforms: { positions: { value: positionsTexture }, uTime: { value: 0 }, uNoiseScale: { value: 1.0 }, uNoiseIntensity: { value: 0.5 }, uTimeScale: { value: 1 }, uLoopPeriod: { value: 24.0 } },
    });
  }
}
